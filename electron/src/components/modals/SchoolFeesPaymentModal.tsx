...
const SchoolFeesPaymentModal: React.FC<SchoolFeesPaymentModalProps> = ({
  ...
  // Charger le numéro WhatsApp de l'école sauvegardé et initialiser le formulaire
  React.useEffect(() => {
    ...
    // Si des données d'élève sont fournies (mode édition), initialiser le formulaire
    if (studentData) {
      ...
      const studentInfo = {
        ...
        name: studentData.student?.name || studentData.studentName || 'Étudiant',
        class: studentData.student?.class || studentData.className || 'Non spécifiée',
        matricule: studentData.student?.matricule || studentData.studentId || studentData.id,
        parentPhone: studentData.student?.parentPhone || studentData.parentPhone,
      ...
      const paymentDateValue = studentData.date || new Date().toISOString().split('T')[0];
      const paymentTimeValue = studentData.time || new Date().toTimeString().slice(0, 5);
      const paymentMethodValue = studentData.method || studentData.paymentMethod || 'mobile_money';
      ...
      setReduction(Number(studentData.reduction) || 0);
      ...
      if (paymentMethodValue === 'mobile_money') {
        setSenderPhone(studentData.senderPhone || studentData.phoneNumber || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedStudent && paymentAmount > selectedStudent.totalRemaining + (studentData?.amount || 0)) {
        throw new Error('Le montant ne peut pas dépasser le montant dû par l\'élève');
      }
      
      // Préparer les données du paiement
      const paymentData = {
        id: studentData?.id || undefined, // Conserver l'ID en mode édition
        ...
        ...(paymentMethod === 'mobile_money' && {
          ...
          receiverPhone: receiverPhone || schoolWhatsAppNumber,
        ...
        // En mode édition, conserver les champs existants qui ne sont pas modifiés
        ...(studentData && {
          // Ajouter d'autres champs existants qui doivent être conservés
          createdAt: studentData.createdAt,
          updatedAt: new Date().toISOString(),
        }),
      };
      
      console.log('Sending payment data:', paymentData); // Pour le débogage
      
      // Simuler l'appel API (à remplacer par un véritable appel API)
      ...
      setError(err instanceof Error ? err.message : 'Une erreur est survenue lors de l\'enregistrement du paiement');
    } finally {
